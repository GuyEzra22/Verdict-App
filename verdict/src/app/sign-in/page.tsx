...
const SignIn: React.FC = () => {
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (userDoc.exists()) {
        console.log(`Welcome ${userDoc.data().name} ${userDoc.data().lastName}`);
      }
  ...
  return (
    <div className="flex flex-col min-h-screen bg-cover bg-center" style={{ backgroundImage: "url('/signin.jpg')" }}>
      <Navbar userName={null} onSignOut={() => {}} />
      <div className="flex items-center justify-center flex-grow">
        <div className="bg-white bg-opacity-90 p-12 rounded-3xl shadow-lg w-full max-w-md">
          ...
          <p className="text-sm text-gray-600 text-center mt-6">
            Don't have an account? <a href="/sign-up" className="text-indigo-600 hover:underline font-medium">Sign Up</a>
          </p>
        </div>
      </div>
    </div>
